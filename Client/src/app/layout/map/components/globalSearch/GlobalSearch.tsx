...
export default function GlobalSearch(props: Readonly<Props>) {
  ...
  const { query, setQuery, results, handleSearch, isFocused, setIsFocused } =
    useSearch();
  ...
  const handleGoTo = useCallback(
    ...
    [flyTo, setQuery, setIsFocused],
  );

  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLButtonElement>, feature: GeoJSON.Feature) => {
      if (e.key === "Enter") {
        handleGoTo(feature);
      }
    },
  ...
  return (
    <>
      <div className={`info-card ${isFocused ? "" : "info-card-hidden"}`}>
        ...
        <div className="shortcut">
          Press <kbd>Esc</kbd> to unfocus the search
        </div>
        <div className="hint">
          Keywords: bina, trafo, yol, direk, agdirek, ogmusdirek, ayddirek, hat,
          aghat, oghat, rekortman
        </div>
      </div>
      <div
        id="global-search"
          ...
          <div className="results">
            {results.map((result) => (
              ...
              >
                ...
                <div className="text">
                  <div className="title" title={result.title}>
                    {result.title}
                  </div>
                  <div className="subtitle" title={result.subtitle}>
                    {result.subtitle}
                  </div>
                </div>
                <div
                  className="position"
                  title={`Position: ${result.position[0]}, ${result.position[1]}`}
                >
