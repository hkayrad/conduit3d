...
export default function MapSettings() {
  ...
  const getHexFromConfig = useCallback((value: string): string => {
    return value.length >= 7 ? value.slice(0, 7) : "#000000";
  }, []);

  const getAlphaFromConfig = useCallback((value: string): number => {
    if (value.length === 9) {
  ...
  const createColorWithAlpha = useCallback(
  ...
  const handleColorChange = useCallback(
    ...
    [dispatch, config, createColorWithAlpha, getAlphaFromConfig],
  );
  ...
  const handleAlphaChange = useCallback(
    ...
    [dispatch, config, createColorWithAlpha, getHexFromConfig],
  );

  // Cleanup timeouts on unmount
  useEffect(() => {
    const timeouts = Object.values(debounceTimeouts.current);
    return () => {
          ...
          >
            ...
            <div className="window-content">
              <div className="color-settings">
                {config &&
                  Object.entries(categorizeColorSettings()).map(
                    ([category, settings]) => (
                              ...
                              <div className="color-controls">
                                ...
                                <div className="alpha-control">
                                  ...
                                  <span className="alpha-value">
                                    {Math.round(
                                      getAlphaFromConfig(setting) * 100,
                                    )}
                                    %
                                  </span>
                                </div>
