...
function convertDeckGLToLatLonWithOffset(
  ...
  const METERS_TO_DEGREES_LON =
    1 / (111320 * Math.cos((baseLatitude * Math.PI) / 180));
...
export default function StaticStreetView() {
  ...
  const [updatedLonLat, setUpdatedLonLat] = useState<{
    latitude: number;
    longitude: number;
  } | null>(null);
  ...
  const fetchImg = useMemo(() => {
    ...
    firstPerson,
    selectedViewType,
    updatedLonLat?.latitude,
    updatedLonLat?.longitude,
  ]);
  ...
  useEffect(() => {
    if (firstPerson && selectedViewType === "firstPerson") {
      ...
      const { latitude, longitude } = convertDeckGLToLatLonWithOffset(
    ...
    <>
      ...
      >
        <button
          ...
          onClick={onPin}
          title="Toggle Large View"
        >
          {isStreetViewPinned ? (
            <PinOff />
          ) : (
            <PinIcon style={{ rotate: "45deg" }} />
          )}
        </button>
        <img
