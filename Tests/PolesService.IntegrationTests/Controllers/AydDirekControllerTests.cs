...
public class AydDirekControllerTests : IClassFixture<WebApplicationFactory<Program>>, IAsyncLifetime
{
    ...
    public AydDirekControllerTests(WebApplicationFactory<Program> factory, ITestOutputHelper output)
    {
        _output = output;
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/AydDirek?pageNumber=1&pageSize=3&minX=-180&minY=-90&maxX=180&maxY=90");

        // Assert
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadFromJsonAsync<Response<List<AydDirek>>>(_jsonOptions);
    ...
    public async Task GetByIdAsync_WithExistingId_ReturnsPole()
    {
        ...
        // Act
        var response = await _client.GetAsync($"/api/v1/AydDirek/{existingId}");
    ...
    public async Task GetByIdAsync_WithNonExistingId_ReturnsNotFoundResponse()
    {
        ...
        // Act
        var response = await _client.GetAsync($"/api/v1/AydDirek/{nonExistingId}");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/AydDirek/count?minX=-180&minY=-90&maxX=180&maxY=90");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/AydDirek/types");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/AydDirek/pbf?minX=-180&minY=-90&maxX=180&maxY=90");
        ...
        var pbfResponse = AydDirekResponse.Parser.ParseFrom(contentBytes);
