...
public class AydDirekRepositoryTests : IAsyncLifetime
{
    private readonly ITestOutputHelper _output;
    private readonly PostgreSqlContainer _postgresContainer;
    private IServiceProvider _serviceProvider = null!;

    public AydDirekRepositoryTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    public async Task InitializeAsync()
    {
        ...
        services.AddScoped<IAydDirekRepository, AydDirekRepository>();
        ...
        try
        {
            ...
            await context.Database.ExecuteSqlRawAsync(@"DROP TABLE IF EXISTS ""SBK_AYDDIREK"" CASCADE;");
            ...
            await context.Database.ExecuteSqlRawAsync(@"
                CREATE TABLE IF NOT EXISTS public.""SBK_AYDDIREK"" (
                    ...
                    CONSTRAINT ""SBK_AYDDIREK_pkey"" PRIMARY KEY (id)
                )
            ");
            ...
            _output.WriteLine("SBK_AYDDIREK table created");

            var canConnect = await context.Database.CanConnectAsync();
            _output.WriteLine($"Can connect to database: {canConnect}");

            await context.Database.ExecuteSqlRawAsync(@"
                INSERT INTO ""SBK_AYDDIREK""(geometry, kodu, adi, cinsi, tipi, direk_no, boy_ozellik, direk_boy_id)
                VALUES (ST_GeomFromText('POINT(4595628.03 4851138.0794)', 3857), 'DIREK-001', 'Direk 1', 'Demir', 'Tip 1', 'No 1', 'Boy 1', 1);
            ");
            await context.Database.ExecuteSqlRawAsync(@"
                INSERT INTO ""SBK_AYDDIREK""(geometry, kodu, adi, cinsi, tipi, direk_no, boy_ozellik, direk_boy_id)
                VALUES (ST_GeomFromText('POINT(4595629.03 4851139.0794)', 3857), 'DIREK-002', 'Direk 2', 'Beton', 'Tip 2', 'No 2', 'Boy 2', 2);
            ");
            await context.Database.ExecuteSqlRawAsync(@"
                INSERT INTO ""SBK_AYDDIREK""(geometry, kodu, adi, cinsi, tipi, direk_no, boy_ozellik, direk_boy_id)
                VALUES (ST_GeomFromText('POINT(4595630.5 4851181.1)', 3857), 'DIREK-003', 'Direk 3', 'Ahşap', 'Tip 3', 'No 3', 'Boy 3', 3);
            ");
        }
        catch (Exception ex)
        {
    ...
    {
        using var scope = _serviceProvider.CreateScope();
        var repository = scope.ServiceProvider.GetRequiredService<IAydDirekRepository>();
    ...
    public async Task GetCountAsync_WithSearchText_ShouldReturnFilteredCount()
    {
        ...
        // Act
        var count = await repository.GetCountAsync(TestDataGenerator.GenerateExtent(), "Beton", CancellationToken.None);
    ...
    public async Task AddAsync_WithValidEntity_ShouldAddToDatabase()
    {
        ...
        var newPole = new AydDirek
        {
            Id = 0,
            Kodu = "AYDDIREK-004",
            Adi = "Test Ayd Direk 4",
            Cinsi = "Beton",
            Tipi = "Test Tipi",
            DirekNo = "DN-004",
            BoyOzellik = "Test",
        ...
        result.Kodu.Should().Be("AYDDIREK-004");
        result.Adi.Should().Be("Test Ayd Direk 4");
    }
    ...
    public async Task UpdateAsync_WithValidEntity_ShouldUpdateInDatabase()
    {
        ...
        existingPole!.Adi = "Updated Ayd Direk Name";
        ...
        // Assert
        result.Should().NotBeNull();
        result.Adi.Should().Be("Updated Ayd Direk Name");
    ...
    public async Task UpdateAsync_WithNonExistentEntity_ShouldThrowException()
    {
        ...
        var nonExistentPole = new AydDirek
        {
            Id = 999,
            Kodu = "AYDDIREK-999",
